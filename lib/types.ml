...
type comparison = EQ | NEQ | GT | GE | LT | LE
...
let pp_fn : printer ref =
  ref
    {
      pp_expr =
        (fun (type a) _fmt (expr : a expr) : unit ->
...
type collector = {
  values_expr : 'a. wrapped_value list -> 'a expr -> wrapped_value list;
}

let values_fn : collector ref =
  ref
    {
      values_expr =
        (fun (type a) _ (expr : a expr) ->
          Format.ksprintf failwith
            "could not resolve collector for expression %s"
            (Obj.Extension_constructor.name
               (Obj.Extension_constructor.of_val expr)));
    }
...
let add_collector f =
  let old_f = !values_fn in
  values_fn := f old_f

type typer = { ty_expr : 'a. 'a expr -> 'a Type.t }

let typer_fn : typer ref =
  ref
    {
      ty_expr =
        (fun (type a) (expr : a expr) ->
...
let () =
  add_collector @@ fun collector ->
  {
    values_expr =
      (fun (type a) acc (expr : a expr) : wrapped_value list ->
        match expr with
        | NULL _ -> acc
...
let rec pp_expr_list_inner : 'a. Format.formatter -> 'a expr_list -> unit =
 fun fmt (type a) (ls : a expr_list) ->
  match ls with
  | [] -> ()
  | h :: t -> Format.fprintf fmt ", %a%a" pp_expr h pp_expr_list_inner t
...
and pp_returning : 'a. Format.formatter -> 'a expr_list -> unit =
 fun fmt ->
  pp_opt_expr_list
    (fun fmt -> Format.fprintf fmt "RETURNING %a" pp_expr_list)
    fmt

and pp_ordering_expr_list_inner :
    'a. [> `ASC | `DESC ] -> Format.formatter -> 'a expr_list -> unit =
 fun ordering fmt (type a) (ls : a expr_list) ->
  match ls with
  | [] -> ()
  | h :: t ->
      Format.fprintf fmt ", %a %a%a" pp_expr h pp_ordering ordering
        (pp_ordering_expr_list_inner ordering)
        t

and pp_ordering_expr_list :
    'a. [> `ASC | `DESC ] -> Format.formatter -> 'a expr_list -> unit =
 fun ordering fmt (type a) (ls : a expr_list) ->
  match ls with
  | [] -> ()
...
and pp_query : 'a 'b. Format.formatter -> ('a, 'b) query -> unit =
 fun fmt (type a b) (query : (a, b) query) ->
  match query with
  | SELECT_CORE { exprs; table; join; where; group_by; having } ->
      Format.fprintf fmt "SELECT %a\nFROM %s%a%a%a%a" pp_expr_list exprs
        (snd table) pp_join_list join
        (pp_opt (fun fmt vl -> Format.fprintf fmt "WHERE %a" pp_expr vl))
        where
        (pp_opt (fun fmt vl -> Format.fprintf fmt "GROUP BY %a" pp_expr_list vl))
        group_by
        (pp_opt (fun fmt vl -> Format.fprintf fmt "HAVING %a" pp_expr vl))
        having
  | SELECT { core; order_by; limit; offset } ->
      Format.fprintf fmt "%a%a%a%a" pp_query core
        (pp_opt (fun fmt (order, vl) ->
             Format.fprintf fmt "ORDER BY %a" (pp_ordering_expr_list order) vl))
        order_by
        (pp_opt (fun fmt vl -> Format.fprintf fmt "LIMIT %a" pp_expr vl))
        limit
        (pp_opt (fun fmt vl -> Format.fprintf fmt "OFFSET %a" pp_expr vl))
        offset
  | DELETE { table; where; returning } ->
      Format.fprintf fmt "DELETE FROM %s%a%a" (snd table)
        (pp_opt (fun fmt vl -> Format.fprintf fmt "WHERE %a" pp_expr vl))
        where pp_returning returning
  | UPDATE { table; on_err; set; where; returning } ->
      Format.fprintf fmt "UPDATE%a %s\nSET %a%a%a" (pp_opt pp_on_err) on_err
        (snd table)
  ...
  | INSERT { table; on_err; on_conflict; set; returning } ->
      let pp_field : 'a. Format.formatter -> 'a expr -> unit =
       fun fmt (type a) (expr : a expr) : unit ->
        ...
        | _ ->
            Format.kasprintf failwith "expected field for INSERT query, got %a"
              pp_expr expr
      in
      Format.fprintf fmt "INSERT%a INTO %s (%a) VALUES (%a)%a%a"
        (pp_opt pp_on_err) on_err (snd table)
        (Format.pp_print_list
           ~pp_sep:(fun fmt () -> Format.fprintf fmt ", ")
           (fun fmt (ASSIGN (fld, _)) ->
             Format.fprintf fmt "%a" pp_field (FIELD fld)))
        set
        (Format.pp_print_list
           ~pp_sep:(fun fmt () -> Format.fprintf fmt ", ")
           (fun fmt (ASSIGN (_, expr)) -> Format.fprintf fmt "%a" pp_expr expr))
        set (pp_opt pp_on_conflict) on_conflict pp_returning returning

and pp_join : int -> Format.formatter -> join -> unit =
 fun n fmt (MkJoin { table; on; join_op }) ->
  Format.fprintf fmt "%a (%a) AS join_tmp_%d ON %a" pp_join_op join_op pp_query
    table n pp_expr on
...
and pp_join_list : Format.formatter -> join list -> unit =
 fun fmt ls ->
  match ls with
  | [] -> ()
  | h :: t -> Format.fprintf fmt " %a%a" (pp_join 0) h (pp_join_list_inner 1) t
...
and query_values :
    'a 'b. wrapped_value list -> ('a, 'b) query -> wrapped_value list =
 fun acc (type a b) (query : (a, b) query) ->
  ...
  | SELECT_CORE { exprs; table = _; join; where; group_by; having } ->
      ...
      let acc =
        List.fold_left
          (fun acc (MkJoin { table; on; join_op = _ }) ->
      ...
      in
      let acc =
        Option.map (values_expr acc) where |> Option.value ~default:acc
      in
      let acc =
        Option.map (values_expr_list acc) group_by |> Option.value ~default:acc
      in
      let acc =
        Option.map (values_expr acc) having |> Option.value ~default:acc
      in
      acc
  | SELECT { core; order_by; limit; offset } ->
      let acc = query_values acc core in
      let acc =
        Option.map (fun (_, expr) -> values_expr_list acc expr) order_by
        |> Option.value ~default:acc
      in
      let acc =
        Option.map (values_expr acc) limit |> Option.value ~default:acc
      in
      let acc =
        Option.map (values_expr acc) offset |> Option.value ~default:acc
      in
      acc
  ...
  | UPDATE { table = _; on_err = _; set; where; returning } ->
      let acc =
        List.fold_left
          (fun acc (ASSIGN (vl, expr)) ->
            values_expr (values_expr acc (FIELD vl)) expr)
          acc set
      in
      let acc =
        Option.map (values_expr acc) where |> Option.value ~default:acc
  ...
  | INSERT { table = _; on_err = _; on_conflict = _; set; returning } ->
      let acc =
        List.fold_left
          (fun acc (ASSIGN (vl, _)) -> values_expr acc (FIELD vl))
          acc set
      in
      let acc =
        List.fold_left
          (fun acc (ASSIGN (_, expr)) -> values_expr acc expr)
          acc set
      in
      let acc = values_expr_list acc returning in
      acc
...
module Common = struct
  ...
  let () =
    ...
    {
      pp_expr =
        (fun (type a) fmt (expr : a expr) : unit ->
          match expr with
          | CAST (expr, ty) ->
              Format.fprintf fmt "CAST(%a AS %s)" pp_expr expr (Type.show ty)
          | BETWEEN (expr, lower, upper) ->
              Format.fprintf fmt "%a BETWEEN %a AND %a" pp_expr expr pp_expr
                lower pp_expr upper
          | NOT_BETWEEN (expr, lower, upper) ->
              Format.fprintf fmt "%a NOT BETWEEN %a AND %a" pp_expr expr pp_expr
                lower pp_expr upper
          | COUNT (false, exprs) ->
          ...
          | COMPARE (op, l, r) ->
              let op =
                ...
                | LE -> "<="
              in
              Format.fprintf fmt "%a %s %a" pp_expr l op pp_expr r
          ...
          | EXISTS query -> Format.fprintf fmt "EXISTS (%a)" pp_query query
          | IN (expr, query) ->
  ...
  let () =
    ...
    {
      values_expr =
        (fun (type a) acc (e : a expr) : wrapped_value list ->
          match e with
          ...
          | SUB (_, l, r) | ADD (_, l, r) | MUL (_, l, r) | DIV (_, l, r) ->
          ...
          | OR (l, r) | AND (l, r) -> values_expr (values_expr acc l) r
          | NOT expr -> values_expr acc expr
          | COMPARE (_, l, r) -> values_expr (values_expr acc l) r
          | IS_NULL expr -> values_expr acc expr
          | IS_NOT_NULL expr -> values_expr acc expr
          | LOWER expr | UPPER expr -> values_expr acc expr
          | LIKE (l, r) -> values_expr (values_expr acc l) r
          | COALESCE exprs -> List.fold_left values_expr acc exprs
          | NULLABLE expr -> values_expr acc expr
          | EXISTS query -> query_values acc query
          | IN (expr, query) -> query_values (values_expr acc expr) query
...
module Postgres = struct
  type 'a expr +=
    ...
    | SUBSTRING :
        string expr * int expr option * int expr option
        -> string expr (* first int is from, second int is for *)
    | REPLACE :
        string expr * string expr * string expr
        -> string expr (* args (string,from,to) *)
    | REVERSE : string expr -> string expr
    | STARTS_WITH : string expr * string expr -> bool expr
    | SIMILAR_TO :
        string expr * string expr
        -> bool expr (* args(string, pattern) *)
  ...
  let () =
    ...
    {
      pp_expr =
        (fun (type a) fmt (e : a expr) : unit ->
          match e with
          | BETWEEN_SYMMETRIC (expr, lower, upper) ->
              Format.fprintf fmt "%a BETWEEN SYMMETRIC %a AND %a" pp_expr expr
                pp_expr lower pp_expr upper
          | NOT_BETWEEN_SYMMETRIC (expr, lower, upper) ->
              Format.fprintf fmt "%a NOT BETWEEN SYMMETRIC %a AND %a" pp_expr
                expr pp_expr lower pp_expr upper
          | IS_DISTINCT_FROM (l, r) ->
              Format.fprintf fmt "%a IS DISTINCT FROM %a" pp_expr l pp_expr r
          | IS_NOT_DISTINCT_FROM (l, r) ->
              Format.fprintf fmt "%a IS NOT DISTINCT FROM %a" pp_expr l pp_expr
                r
          | IS_TRUE expr -> Format.fprintf fmt "%a IS TRUE" pp_expr expr
          | IS_NOT_TRUE expr -> Format.fprintf fmt "%a IS NOT TRUE" pp_expr expr
          | IS_FALSE expr -> Format.fprintf fmt "%a IS FALSE" pp_expr expr
          | IS_NOT_FALSE expr ->
              Format.fprintf fmt "%a IS NOT FALSE" pp_expr expr
          | IS_UNKNOWN expr -> Format.fprintf fmt "%a IS UNKNOWN" pp_expr expr
          | IS_NOT_UNKNOWN expr ->
              Format.fprintf fmt "%a IS NOT UNKNOWN" pp_expr expr
          ...
          | LOG (base, x) ->
              Format.fprintf fmt "LOG(%a,%a)" pp_expr base pp_expr x
          | POWER (_, a, b) ->
              Format.fprintf fmt "POWER(%a,%a)" pp_expr a pp_expr b (* a ^ b *)
          ...
          | FACTORIAL (_, expr) ->
              Format.fprintf fmt "FACTORIAL(%a)" pp_expr expr
          ...
          | CONCAT exprs ->
              Format.fprintf fmt "CONCAT(%a)"
                ...
                exprs
          | CONCAT_WS (sep, []) ->
              Format.fprintf fmt "CONCAT_WS(%a)" pp_expr sep
          | CONCAT_WS (sep, exprs) ->
          ...
          | SUBSTRING (text, None, None) ->
              Format.fprintf fmt "SUBSTRING(%a)" pp_expr text
          | SUBSTRING (text, Some from, None) ->
              Format.fprintf fmt "SUBSTRING(%a FROM %a)" pp_expr text pp_expr
                from
          | SUBSTRING (text, None, Some for_) ->
              Format.fprintf fmt "SUBSTRING(%a FOR %a)" pp_expr text pp_expr
                for_
          | SUBSTRING (text, Some from, Some for_) ->
              Format.fprintf fmt "SUBSTRING(%a FROM %a FOR %a)" pp_expr text
                pp_expr from pp_expr for_
          | REPLACE (text, from, to_) ->
              Format.fprintf fmt "REPLACE(%a,%a,%a)" pp_expr text pp_expr from
                pp_expr to_
          | REVERSE expr -> Format.fprintf fmt "REVERSE(%a)" pp_expr expr
          | STARTS_WITH (text, prefix) ->
              Format.fprintf fmt "STARTS_WITH(%a,%a)" pp_expr text pp_expr
                prefix
          | SIMILAR_TO (text, pat) ->
              Format.fprintf fmt "(%a) SIMILAR TO (%a)" pp_expr text pp_expr pat
          | GREATEST (_, exprs) ->
    ...
    add_collector @@ fun collector ->
    {
      values_expr =
        (fun (type a) acc (e : a expr) : wrapped_value list ->
          ...
          | NOT_BETWEEN_SYMMETRIC (expr, lower, upper) ->
              values_expr (values_expr (values_expr acc expr) lower) upper
          | IS_DISTINCT_FROM (l, r) -> values_expr (values_expr acc l) r
          | IS_NOT_DISTINCT_FROM (l, r) -> values_expr (values_expr acc l) r
          ...
          | CONCAT_WS (sep, exprs) ->
              List.fold_left
                (fun acc expr -> values_expr acc expr)
          ...
          | SUBSTRING (text, Some from, None) ->
              values_expr (values_expr acc text) from
          | SUBSTRING (text, None, Some for_) ->
              values_expr (values_expr acc text) for_
          | SUBSTRING (text, Some from, Some for_) ->
              values_expr (values_expr (values_expr acc text) from) for_
          | REPLACE (text, from, to_) ->
          ...
          | SIMILAR_TO (text, pat) -> values_expr (values_expr acc text) pat
          | GREATEST (_, exprs) ->
              List.fold_left (fun acc expr -> values_expr acc expr) acc exprs
          | LEAST (_, exprs) ->
              List.fold_left (fun acc expr -> values_expr acc expr) acc exprs
    ...
    add_typer @@ fun typer ->
    {
      ty_expr =
        (fun (type a) (e : a expr) : a Type.t ->
          ...
          | GREATEST (_, []) ->
              failwith "invalid syntax -- empty GREATEST expression"
  ...
  let () =
    add_printer @@ fun pp ->
    {
      pp_expr =
        (fun (type a) fmt (expr : a expr) : unit ->
          match expr with
          | TOTAL (false, expr) -> Format.fprintf fmt "TOTAL(%a)" pp_expr expr
          | TOTAL (true, expr) ->
          ...
          | GROUP_CONCAT (true, l, Some r) ->
              Format.fprintf fmt "GROUP_CONCAT(DISTINCT %a, %a)" pp_expr l
                pp_expr r
    ...
    add_collector @@ fun collector ->
    {
      values_expr =
        (fun (type a) acc (e : a expr) : wrapped_value list ->
          ...
          | GROUP_CONCAT (_, l, None) -> values_expr acc l
          | GROUP_CONCAT (_, l, Some r) -> values_expr (values_expr acc l) r
          | CHANGES -> acc
          | GLOB (l, r) -> values_expr (values_expr acc l) r
          | MIN_OF (_, exprs) -> List.fold_left values_expr acc exprs
          | MAX_OF (_, exprs) -> List.fold_left values_expr acc exprs
          | RANDOM -> acc
          | _ -> collector.values_expr acc e);
    }
