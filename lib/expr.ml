...
type wrapped_value = Types.wrapped_value =
  | MkWrapped : 'a Type.t * 'a -> wrapped_value
...
module Common = struct
  ...
  let as_ expr ~name =
    (Types.AS (expr, name), Types.REF (name, Types.ty_expr expr))
  ...
  let null : 'a. 'a Type.t -> 'a option Type.t =
   fun (type a) (ty : a Type.t) : a option Type.t -> Type.null_ty ty
  ...
  let max ?(distinct = false) expr =
    Types.Common.MAX (distinct, Type.Numeric.Int, expr)

  let min ?(distinct = false) expr =
    Types.Common.MIN (distinct, Type.Numeric.Int, expr)

  let sum ?(distinct = false) expr =
    Types.Common.SUM (distinct, Type.Numeric.Int, expr)

  let maxf ?(distinct = false) expr =
    Types.Common.MAX (distinct, Type.Numeric.Real, expr)

  let minf ?(distinct = false) expr =
    Types.Common.MIN (distinct, Type.Numeric.Real, expr)

  let sumf ?(distinct = false) expr =
    Types.Common.SUM (distinct, Type.Numeric.Real, expr)
...
module Postgres = struct
  ...
  let between_symmetric ~lower ~upper x =
    Types.Postgres.BETWEEN_SYMMETRIC (x, lower, upper)

  let not_between_symmetric ~lower ~upper x =
    Types.Postgres.NOT_BETWEEN_SYMMETRIC (x, lower, upper)
...
module Sqlite3 = struct
  ...
  let group_concat ?(distinct = false) ?sep_by l =
    Types.Sqlite3.GROUP_CONCAT (distinct, l, sep_by)
