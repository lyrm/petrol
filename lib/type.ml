 ...
 fun (type a) ~(ty : a Caqti_type.t) ~repr : a t ->
  ...
  let eq_witness : 'b. 'b witness -> (a, 'b) eq option =
   fun (type b) (witness : b witness) : (a, b) eq option ->
    match witness with M.CUSTOM -> Some Refl | _ -> None
  in
        ...
        eq_witness =
          {
            eq =
              (fun (type b) (witness : b witness) : (int64, b) eq option ->
                match witness with BIGINT -> Some Refl | _ -> None);
          };
      }
        ...
        eq_witness =
          {
            eq =
              (fun (type b) (witness : b witness) : (int64, b) eq option ->
                match witness with BIGSERIAL -> Some Refl | _ -> None);
          };
      }
        ...
        eq_witness =
          {
            eq =
              (fun (type b) (witness : b witness) : (string, b) eq option ->
                match witness with BYTEA -> Some Refl | _ -> None);
          };
      }
        ...
        eq_witness =
          {
            eq =
              (fun (type b) (witness : b witness) : (Ptime.t, b) eq option ->
                match witness with DATE -> Some Refl | _ -> None);
          };
      }
        ...
        eq_witness =
          {
            eq =
              (fun (type b) (witness : b witness) : (float, b) eq option ->
                match witness with DOUBLE_PRECISION -> Some Refl | _ -> None);
          };
      }
        ...
        eq_witness =
          {
            eq =
              (fun (type b) (witness : b witness) : (int32, b) eq option ->
                match witness with INT4 -> Some Refl | _ -> None);
          };
      }
        ...
        eq_witness =
          {
            eq =
              (fun (type b) (witness : b witness) : (int, b) eq option ->
                match witness with SMALLINT -> Some Refl | _ -> None);
          };
      }
        ...
        eq_witness =
          {
            eq =
              (fun (type b) (witness : b witness) : (int, b) eq option ->
                match witness with SMALLSERIAL -> Some Refl | _ -> None);
          };
      }
        ...
        eq_witness =
          {
            eq =
              (fun (type b) (witness : b witness) : (Ptime.t, b) eq option ->
                match witness with TIME -> Some Refl | _ -> None);
          };
      }
end
        ...
        eq_witness =
          {
            eq =
              (fun (type b) (witness : b witness) : (string, b) eq option ->
                match witness with BLOB -> Some Refl | _ -> None);
          };
      }
end
...
let rec ty_list_to_caqti_ty : 'a. 'a ty_list -> 'a Caqti_type.t =
 fun (type a) (ls : a ty_list) : a Caqti_type.t ->
  match ls with
  | Nil -> Caqti_type.unit
  | Cons (h, t) -> Caqti_type.t2 (ty_to_caqti_ty h) (ty_list_to_caqti_ty t)
...
let rec eq_ty : 'a 'b. 'a t * 'b t -> ('a, 'b) eq option =
 fun (type a b) ((e1, e2) : a t * b t) : (a, b) eq option ->
  ...
  | NULLABLE l, NULLABLE r -> (
      match eq_ty (l, r) with Some Refl -> Some Refl | None -> None)
  | _ -> None
...
let rec eq_ty_list : 'a 'b. 'a ty_list * 'b ty_list -> ('a, 'b) eq option =
 fun (type a b) ((l1, l2) : a ty_list * b ty_list) : (a, b) eq option ->
  ...
  | Cons (h1, t1), Cons (h2, t2) -> (
      ...
      | Some Refl -> (
          match eq_ty_list (t1, t2) with Some Refl -> Some Refl | None -> None)
      | None -> None)
  | _ -> None
...
let rec pp_value : 'a. 'a t -> Format.formatter -> 'a -> unit =
 fun (type a) (ty : a t) fmt (vl : a) ->
  ...
  | TEXT -> Format.fprintf fmt "%s" vl [@alert "-deprecated"]
  ...
  let rec lookup_opt : 'a t -> key:'k ty_list -> ('k, 'a) S.t option =
    let handle :
     ...
     fun (type a b) (l1 : b ty_list) ((l2, data) : a ty_list * (a, 'opt) S.t) :
         (b, 'opt) S.t option ->
      match eq_ty_list (l1, l2) with Some Refl -> Some data | _ -> None
    in
    fun ls ~key ->
      match ls with
      | [] -> None
