...
type ('a, 'c) where_fun = bool Expr.t -> ('c, 'a) t -> ('c, 'a) t
  constraint 'a = [< `SELECT_CORE | `SELECT | `DELETE | `UPDATE ] as 'a

type ('a, 'b, 'c) group_by_fun = 'b Expr.expr_list -> ('c, 'a) t -> ('c, 'a) t
  constraint 'a = [< `SELECT_CORE | `SELECT ] as 'a
...
let query_ret_ty : 'a 'b. ('a, 'b) t -> 'a Type.ty_list =
 fun (type a b) (query : (a, b) t) : a Type.ty_list ->
  match query with
  | SELECT_CORE
      { exprs; table = _; join = _; where = _; group_by = _; having = _ } ->
...
let where : ('a, 'c) where_fun =
 fun by (type a b) (table : (b, a) t) : (b, a) t ->
  let update_where where by =
    match where with
    | None -> Some by
    | Some old_by -> Some Expr.Common.(by && old_by)
  in
  match table with
  ...
  | Types.SELECT
      {
        core = SELECT_CORE { exprs; table; join; where; group_by; having };
        order_by;
        limit;
        offset;
      } ->
...
let group_by : ('a, 'b, 'c) group_by_fun =
 fun by (type a b) (table : (b, a) t) : (b, a) t ->
  ...
  | Types.SELECT
      {
        core = SELECT_CORE { exprs; table; join; where; group_by = _; having };
        order_by;
        limit;
        offset;
      } ->
      SELECT
        {
          core =
            SELECT_CORE
              { exprs; table; join; where; group_by = Some by; having };
          order_by;
          limit;
          offset;
        }
  | Types.DELETE _ | Types.UPDATE _ | Types.INSERT _ ->
...
let having : ('a, 'c) having_fun =
 fun having (type a b) (table : (b, a) t) : (b, a) t ->
  ...
  | Types.SELECT
      {
        core = SELECT_CORE { exprs; table; join; where; group_by; having = _ };
        order_by;
        limit;
        offset;
      } ->
      SELECT
        {
          core =
            SELECT_CORE
              { exprs; table; join; where; group_by; having = Some having };
          order_by;
          limit;
          offset;
        }
...
let join : ('a, 'b, 'd, 'c) join_fun =
 fun ?(op = INNER) ~on (type a b c) (ot : (b, _) t) (table : (c, a) t) ->
  match table with
  | Types.SELECT_CORE { exprs; table; join; where; group_by; having } ->
      Types.SELECT_CORE
        {
          exprs;
          table;
          join = join @ [ MkJoin { table = ot; on; join_op = op } ];
          where;
          group_by;
          having;
        }
  | Types.SELECT _ | Types.DELETE _ | Types.UPDATE _ | Types.INSERT _ ->
      invalid_arg "group by only supported on select clause"
...
let on_err :
 ...
 fun on_err (type a) (table : (_, a) t) : (_, a) t ->
  match table with
  | Types.SELECT_CORE _ | Types.SELECT _ | Types.DELETE _ ->
      invalid_arg "on_err only supported for update and insert"
  | Types.UPDATE query -> UPDATE { query with on_err = Some on_err }
  | Types.INSERT query -> INSERT { query with on_err = Some on_err }

let on_conflict : 'a. [ `DO_NOTHING ] -> ('c, 'a) t -> ('c, 'a) t =
 fun on_conflict (type a) (table : (_, a) t) : (_, a) t ->
  match table with
  | Types.SELECT_CORE _ | Types.SELECT _ | Types.UPDATE _ | Types.DELETE _ ->
      invalid_arg "on_conflict only supported for insert"
  | Types.INSERT query -> INSERT { query with on_conflict = Some on_conflict }

let limit :
    'a 'i.
    int Types.expr ->
    ('a, ([< `SELECT | `SELECT_CORE ] as 'i)) t ->
    ('a, [> `SELECT ]) t =
 fun (type a i) by (table : (a, i) t) : (a, [> `SELECT ]) t ->
  ...
  | DELETE _ | UPDATE _ | INSERT _ ->
      invalid_arg "limit only supported for select"
...
let offset :
 ...
 fun (type a i) by (table : (a, i) t) : (a, [> `SELECT ]) t ->
  ...
  | DELETE _ | UPDATE _ | INSERT _ ->
      invalid_arg "offset only supported for select"
...
let order_by :
    ...
    ('a, [> `SELECT ]) t =
 fun (type a b) ?(direction = `ASC) field (table : (a, b) t) :
     (a, [> `SELECT ]) t ->
  match table with
  | Types.SELECT_CORE { exprs; table; join; where; group_by; having } ->
      SELECT
        {
          ...
          limit = None;
          offset = None;
          order_by = Some (direction, Expr.[ field ]);
        }
  | Types.SELECT { core; order_by = _; limit; offset } ->
      SELECT
        { core; order_by = Some (direction, Expr.[ field ]); limit; offset }
  | DELETE _ | UPDATE _ | INSERT _ ->
      invalid_arg "order by only supported for select"
