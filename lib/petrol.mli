...
module Schema : sig
  ...
  val table_foreign_key :
    ...
    string list ->
    [ `Table ] constraint_
  (** [table_foreign_key ?name ?on_update ?on_delete ~table ~columns cols]
...
module Query : sig
  ...
  *)
  ...
  (** [order_by ?direction fields expr] corresponds to the SQL
      [{expr} ORDER BY {direction} {fields}]. *)
...
module VersionedSchema : sig
  ...
  (** [init ?migrations dialect version ~name] constructs a new versioned schema
      declaring it to have the name [name] and version [version] using SQL
      dialect [dialect].

      [name] is the name of the schema -- used to initially determine the stored
      version number, and is required to stay constant over the lifetime of the
      project.

      [version] is the current version of the schema -- note that {!initialise}
      will fail if it is run using an SQL database has a newer version than the
      version declared here.

      [migrations] is an association list, mapping versions to the SQL
      statements required to migrate the schema to the new format from its
      previous version. The order of elements in [migrations] is irrelevant. *)
  ...
  (** [declare_table t ?since ?constraints ?migrations ~name table_spec]
      declares a new table on the schema [t] with the name [name].

      [since] declares the first version in which this table was introduced --
      if omitted, Petrol assumes the table has been present since the very first
      version.

      [constraints] are a list of SQL constraints on the columns of the table.

      [migrations] is an association list, mapping versions to the SQL
      statements required to migrate this table to the new format from its
      previous version. The order of elements in [migrations] is irrelevant. *)
  ...
  val migrations_needed :
    ...
    (module Caqti_lwt.CONNECTION) ->
    ( bool,
      [> Caqti_error.t | `Newer_version_than_supported of version ] )
    Lwt_result.t
  (** [migrations_needed t conn] returns a boolean indicating whether the
      current version on the SQL database will require migrations -- i.e whether
      running {!initialise} will run migrations.

      [migrations_needed] will also fail if it is run using an SQL database has
      a newer version than the version declared here.
  ...
  val initialise :
    ...
    ( unit,
      [> Caqti_error.t | `Newer_version_than_supported of version ] )
    Lwt_result.t
  (** [initialise t conn] initialises the SQL database on [conn], performing any
      necessary migrations if needed.

      [initialise] will fail if it is run using an SQL database has a newer
      version than the version declared here.
