let () =
  Printexc.register_printer (function Failure msg -> Some msg | _ -> None)
...
let main f =
  ...
  run
    (let* conn = Caqti_lwt.connect (Uri.of_string ("sqlite3://:" ^ fname)) in
     let args =
       List.init (Array.length Sys.argv - 3) (fun ind -> Sys.argv.(3 + ind))
     in
...
let main_postgres f =
  ...
  @@
  let args =
    List.init (Array.length Sys.argv - 1) (fun ind -> Sys.argv.(1 + ind))
  in
  match[@warning "-8"] args with
  | [ "createdb"; name ] ->
      let* (module DB) =
        Caqti_lwt.connect (Uri.of_string "postgresql://localhost:5432")
      in
      Lwt.bind
        (DB.exec
           ...
           ())
      @@ fun err ->
      Lwt.bind (DB.disconnect ()) @@ fun () -> Lwt.return err
  | [ "dropdb"; name ] ->
      let* (module DB) =
        Caqti_lwt.connect (Uri.of_string "postgresql://localhost:5432")
      in
      Lwt.bind
        (DB.exec
           (Caqti_request.Infix.(Caqti_type.unit ->. Caqti_type.unit)
              (Format.sprintf
                 {sql| DROP DATABASE IF EXISTS %s WITH (FORCE) |sql} name))
           ())
      @@ fun err ->
      Lwt.bind (DB.disconnect ()) @@ fun () -> Lwt.return err
  | name :: args -> (
      let* ((module DB) as conn) =
        Caqti_lwt.connect
          (Uri.of_string (Format.sprintf "postgresql://localhost:5432/%s" name))
      in
      try
        Lwt.bind
          (match f conn args with
          | computation -> computation
          | exception Match_failure _ -> failwith "unknown query command")
        @@ fun res -> Lwt.return res
      with e ->
        Lwt.bind (DB.disconnect ()) @@ fun () ->
