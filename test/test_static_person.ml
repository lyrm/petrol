...
module Person = struct
        ...
        Schema.
          [
            field
              ...
              "id" ~ty:Type.int;
            field
              ~constraints:[ unique ~name:"unique_names" () ]
              "name" ~ty:Type.text;
  ...
  let random () =
    let random_char () =
      Char.unsafe_chr Char.(code 'a' + Random.int (code 'z' - code 'a' + 1))
    in
    let random_string n = String.init n (fun _ -> random_char ()) in
    { name = random_string 10; age = Random.int 100 }
end

let () =
  Test_utils.main (fun conn -> function [@warning "-8"]
    | "init", _ ->
    ...
    | "find-by", [ name ] ->
        let* _ = Petrol.StaticSchema.initialise db conn in
        let* result = Person.Sql.find_by ~name conn in
        (match result with
    ...
    | "find-older-than", [ age ] ->
        ...
        List.iteri
          (fun ind Person.{ name; age } ->
            print_endline
              ("[" ^ string_of_int ind ^ "] - name: " ^ name ^ "; age: "
             ^ string_of_int age))
          result;
        Lwt.return_ok ()
    | "list", _ ->
